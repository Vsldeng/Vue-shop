<template>
  <div class="rights">
        ...
        <el-table-column type="expand">
          <template v-slot:default="scope">
            <el-row
              ...
              :key="item1.id"
              :class="['bdbottom',i1===0? 'bdtop':0,'vcenter']"
            >
              <!-- 渲染一级权限 -->
              <el-col :span="5">
              ...
              <el-col :span="19">
                <el-row
                  ...
                  :key="item2.id"
                  :class="['bdbottom',i2===0? 'bdtop':0,'vcenter']"
                >
                  <el-col :span="6">
                    <el-tag
                      type="success"
        ...
        <el-table-column label="操作" prop="level" width="350px">
          <template v-slot:default="scope">
            <el-button
              ...
              type="primary"
              icon="el-icon-edit"
              @click="handleRewriteRole(scope.row.id)"
            >编辑</el-button>
            <el-button
              size="mini"
              type="danger"
    ...
    <el-dialog
      title="编辑角色"
      :visible.sync="RewriteRoleVision"
      width="50%"
      @close="RewriteRoleClosed"
    >
      <!-- 内容主体区域 -->
    ...
    handleHandleRewriteRole() {
      this.$refs.RewriteRoleRef.validate(async valid => {
        ...
        const { data: res } = await this.$http.put(
          'roles/' + this.RewriteRole.roleId,
          {
            roleName: this.RewriteRole.roleName,
            roleDesc: this.RewriteRole.roleDesc
          }
        )
        if (res.meta.status !== 200) {
